...
public:
	...
	int value;     // for integers: a value
	string name;	  // for word
	Token(char ch)    // make a Token from a char
		:kind(ch), value(0) { }
	Token(char ch, int val)     // make a Token from a char and an integer
		:kind(ch), value(val) { }
...
int factorial();		// declaration so that primary() can call factorial()
...
int term()
{
	...
	while (true) {
		...
		switch (t.kind) {
		case '*':
			left = narrow_cast<int>(double(left) * factorial());
			t = ts.get();
			break;
		case '/':
		{
...
int expression()
{
	...
	while (true) {
		...
		case '+':
			...
			left = narrow_cast<int>(double(left) + term());
			t = ts.get();
			break;
		case '-':
...
void print_message()
{
	cout << "Welcome to our simple calculator.\n\n"
             << "Please enter expressions using only integer values.\n\n"
             << "Available operators: '*', '/', '+', '-', '!', '%'\n\n"
	     << "Usage: \n\n"
